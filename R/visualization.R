...
svg.2Dplot=function(data,gene,method,title=NULL,log=F){
  ## method= mean/raw
  ...
  if(method=='raw'){
    for (g in gene) {
      g1=ggplot(data=data,aes(col,-row))+   ## aes(col,-row)可以和切片图像方向一致
        geom_point(aes_string(col=g),size=0.5)+
        scale_color_gradient(low = hsv(0.58,0.92,0.92),high = hsv(1,1,1))+
        labs(title=paste0('SVG: ',g))+
        xlab('col')+ylab('row')+
...
svg.Mplot=function(data,gene,method='mean'){
  ...
  if(method=='raw'){
    ...
    for (g in gene) {
      g.row=ggplot(data=new.x,aes_string('Group.1',g))+   ## aes(col,-row)可以和切片图像方向一致
        geom_line(aes_string(col=g))+
        scale_color_gradient(low = hsv(0.58,0.92,0.92),high = hsv(1,1,1))+
        labs(title=paste0('Marginal Distribution  SVG: ',g))+
        xlab('row')+ylab('gene expresson')
      g.col=ggplot(data=new.y,aes_string('Group.1',g))+   ## aes(col,-row)可以和切片图像方向一致
        geom_line(aes_string(col=g))+
        scale_color_gradient(low = hsv(0.58,0.92,0.92),high = hsv(1,1,1))+
        labs(title=paste0('Marginal Distribution  SVG: ',g))+
        xlab('col')+ylab('gene expresson')
  ...
  if(method=='mean'){
    ...
    g.row=ggplot(data=new.x,aes_string('Group.1',g))+   ## aes(col,-row)可以和切片图像方向一致
      geom_line(aes_string(col=g))+
      scale_color_gradient(low = hsv(0.58,0.92,0.92),high = hsv(1,1,1))+
      labs(title=paste0('Marginal Distribution  SVG cluster'))+
      xlab('row')+ylab('gene expresson')
