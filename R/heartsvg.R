...
heartsvg=function (data, scale = T, qh = 0.985, noise = T, padj.m = "holm") {
  ...
  testadjmin=p.adjust(testadj_min = p.adjust(testmin, method = padj.m)
  test = merge(test, sum, by = "gene")
  test = subset(test, zero.p > 0)
  testzero.p=testzero.p = testzero.p/max(test$zero.p)
  testmean=testmean = testmean/max(test$mean)
  testc2=(testc2 = (testzero.p + test$mean)/2
  if (noise == T) {
